...
)
{
	...
	//	Fatt = 1 / Kc*1 + Kl*d + Kq*(d^2)
	//	Kc = constant attenuation factor	(Attenuation.x)
	//	Kl = linear attenuation factor		(Attenuation.y)
	//	Kq = quadratic attenuation factor	(Attenuation.z)
	//	
...
float4 PS( VS_OUTPUT input ) : SV_Target
{
	...
	litColor *= mainTexture.Sample(aSampler, input.Tex);
	...
	float foglerp = saturate((distToEye - FgLight.FogStart) / FgLight.FogRange);	// See https://msdn.microsoft.com/en-us/library/windows/desktop/bb509645(v=vs.85).aspx
	
	//blend litcolor with fog level
	litColor = lerp(litColor, FgLight.FogColor, foglerp);	// See https://msdn.microsoft.com/en-us/library/windows/desktop/bb509618(v=vs.85).aspx
	///========================================================*/
